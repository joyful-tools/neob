...
/**
 * TextShake applies a step-animation shaking effect individually to each letter, preventing word-splitting.
 */
export function TextShake({ text, className }: TextAnimationProperties) {
	const tokens = text.split(/(\s+)/u);

	// Pre-calculate starting globalIndex for each token using a standard sequential loop
	const tokenItems: Array<{ token: string; startIndex: number }> = [];
	...
	for (const token of tokens) {
		...
		currentGlobalIndex += token.length;
	}
	...
	return (
		<>
			...
			<span aria-hidden="true" className={cn('inline-flex flex-wrap select-none', className)}>
				{tokenItems.map((item, tokenIndex) => {
					const { token, startIndex } = item;
					...
					return (
						<span key={tokenIndex} className="inline-block">
							{[...token].map((letter, letterIndex) => {
								...
								const hash = Math.abs(Math.sin(currentIndex + 1) * 10_000) % 1;
...
/**
 * TextWave applies a smooth vertical wave animation individually to each letter, preventing word-splitting.
 */
export function TextWave({ text, className }: TextAnimationProperties) {
	const tokens = text.split(/(\s+)/u);
					...
					return (
						<span key={tokenIndex} className="inline-block">
							{[...token].map((letter, letterIndex) => {
								...
								// Use incremental delay based on letter index to create a wave effect
								const offset = `${currentIndex * -0.1}s`;

								return (
									<span
										key={currentIndex}
										className="neo-wave whitespace-pre"
										style={{
											animationDelay: offset,
										}}
									>
