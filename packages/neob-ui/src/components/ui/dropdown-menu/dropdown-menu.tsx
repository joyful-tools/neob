...
function DropdownMenuItem({ children, className, disabled, onSelect, ref, 'aria-current': ariaCurrent }: DropdownMenuItemProperties) {
	return (
		<Menu.Item
			...
			className={cn(
				`relative flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold text-black outline-hidden transition-all select-none focus:bg-cyan focus:text-black data-disabled:pointer-events-none data-disabled:opacity-disabled dark:text-white dark:focus:text-cyan-light`,
				className,
			)}
		>
			{children}
